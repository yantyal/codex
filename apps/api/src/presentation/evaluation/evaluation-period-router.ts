...
/**
 * 認証済み利用者向け評価期間APIを作成する。
 * @param auth セッションを確認する認証サービス
 * @param service 評価期間を処理するサービス
 * @returns Expressへ登録するルーター
 */
export function createEvaluationPeriodRouter(
  auth: AuthService,
  service: EvaluationPeriodService,
): Router {
  const router = Router();
  ...
  router.get('/:id', async (request, response) => {
    ...
    return value
      ? response.json(value)
      : response.status(404).json({ message: '評価期間が見つかりません。' });
  });
  router.put('/:id', async (request, response) => {
    ...
    return result.ok
      ...
      : response
          .status(result.message === '評価期間が見つかりません。' ? 404 : 400)
