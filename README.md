...
`Run lint` で失敗した場合はコードの書き方、`Check formatting` で失敗した場合は `npm run format` の実行漏れ、`Run tests with coverage` で失敗した場合はテスト結果とカバレッジ結果、`Run audit` で失敗した場合は依存関係の脆弱性を確認します。
