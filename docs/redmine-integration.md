...
- MCP とインポート処理は、対象プロジェクトに所属する専用ユーザー `Codex` の API キーを使用する。
...
起動後、ブラウザで `http://localhost:8080` を開ける。管理者パスワードは `.env.redmine` の `REDMINE_ADMIN_PASSWORD` から確認する。通常のMCP利用では管理者パスワードを使わず、専用ユーザー `Codex` の API キーを使用する。
